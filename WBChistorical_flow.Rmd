...
AddedSites <- matrix(c("SNR1", "13010065", "1984-10-01", "", "Snake River at Flagg, WY",
                       ...
                       "WBC", "09505200", "", "", "Wet Beaver Creek near Rimrock, AZ",
...
historical_stats %>% ggplot(aes(x=Day)) +
  ...
  theme_bw() + 
  labs(x="Days Since January 1st", y="Discharge (cfs)", title=paste0("Historical Hydrograph \n for ", siteName, " (1961-2023)")) +
...
annual_calendar %>% 
  ...
  labs(x="Calendar Year", 
       y="Discharge Sum (cfs)", 
       title=paste0("Annual Hydrograph \n for ", siteName)) +
  theme(plot.title = element_text(hjust=0.5, face="bold"),
        axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"),
...
annual_wateryear %>% 
  ggplot(aes(x=waterYear, y = sum_cfs)) +
  geom_line() +
  geom_smooth(color = "green", se = F) +
  geom_point() +
  theme_bw() +
...
istat_results %>% 
  ...
  geom_segment( aes(x=`Discharge Statistic Name`, xend=`Discharge Statistic Name`, 
                    y=0, yend=slope), color="skyblue") +
  geom_point( color="blue", size=4, alpha=0.6) +
  theme_light() +
  coord_flip() +
  labs(x="Discharge statistic name", y="Slope (cfs/year)")
...
peakFlow_instantaneous <- readNWISpeak(siteID, startDate=startDate, endDate=endDate)
...
theil_sen <- senth(peakFlow$waterYear, peakFlow$wateryear_day, 
                   ...
                   legend_loc = "topright")  
theil_sen <- senth(peakFlow$Year, peakFlow$Day, 
                   site = site, 
                   Xlab = "Calendar Year", 
                   Ylab = "Days Since January 1st", 
                   legend_loc = "topright")  
...
peakFlow %>% 
  ggplot(aes(x=Year, y=Day, color=month)) +
  geom_point()

peakFlow %>% 
  ggplot(aes(x=waterYear, y=month, color=month, size=cfs)) +
  geom_point() +
...
plot(peakFlow$waterYear, peakFlow$wateryear_day, cex.lab=1.25, xlab = "Year", ylab = "Day of the Year", 
