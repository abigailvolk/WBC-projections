...
## 1.2 Set up Matrix with Site Info
...
AddedSites <- matrix(c("WBC", "09505200", "", "2022-10-01", "Wet Beaver Creek near Rimrock, AZ"),
                     ncol = 5, byrow = TRUE)


StreamGageMatrix <- rbind(StreamGageMatrix, AddedSites)
StreamGageMatrix <- data.frame(StreamGageMatrix)
SpecificGageInfo <- subset(StreamGageMatrix, Site == site)
```

## 1.3 Set up EGRET object
...
save_path <- "WBChistorical_flow_output"
```
...
save_figure("hydrograph_ordinaldays.pdf", save_path)
```
...
annual_calendar %>% 
  ...
  nps_theme2()
save_figure("hydrograph_calendaryr_mean.pdf", save_path)
...
annual_calendar %>% 
  ...
  labs(x="Calendar Year", 
       y="Discharge Sum (mm)") +
  nps_theme2()
save_figure("hydrograph_calendaryr_sum.pdf", save_path)
...
annual_wateryear %>% 
  ...
  labs(x="Water Year", 
       y="Discharge Mean (cfs)") +
  nps_theme2()
save_figure("hydrograph_wateryr_mean.pdf", save_path)
...
annual_wateryear %>% 
  ...
  labs(x="Water Year", 
       y="Discharge Sum (mm)") +
  nps_theme2()
save_figure("hydrograph_wateryr_sum.pdf", save_path)
```
...
for (stat in 1:8) {
  ...
  istat <- c(model = site, istat)
  istat_results[[length(istat_results)+1]] <- istat
}
istat_results <- reduce(istat_results, bind_rows) 

istat_table <- as_tibble(istat_table)
istat_table$discharge_stat <- as.numeric(istat_table$discharge_stat)
istat_results <- left_join(istat_results, istat_table, by = "discharge_stat")
```
...
save_figure("istats_histbarchart_slope.pdf", save_path)
```
...
peakflow_season %>% 
  ...
  nps_theme2() 
save_figure("peakflow_day_hist.pdf", save_path)
...
peakflow_season %>% 
  ...
  labs(x="Water Year", y="Days since October 1st") +
  nps_theme2()
save_figure("peakflow_daymonth_hist.pdf", save_path)
...
peakflow_season %>% 
  ...
  geom_point() +
  nps_theme2()
save_figure("peakflow_mag_hist.pdf", save_path)
```
...
```{r Senth-Output, echo=FALSE, message=FALSE, warning=FALSE}
...
for (i in seq_along(seasons)) {  
  ...
  regressions[[i * 2 - 1]] <- day_info   
  regressions[[i * 2]] <- mag_info
}

regressions <- reduce(regressions, bind_rows) %>% 
  relocate(model, discharge_stat, slope, slopepct_mag, 
           slopepct_mag_interp, p_value)
istat_results <- istat_results %>% select(!`Discharge Statistic Name`) %>% 
  rbind(., regressions)
```
...
## 7.2 Saving Data from this Rmd as CSVs

```{r save-csvs, include=FALSE}
path <- "EGRETdaily_historical"
dir.create(path)
#Daily
...
#Peak Flow
write.csv(peakflow_season, file = paste0(path, "/", site, "PeakFlow.csv"))
# Stat results
write.csv(istat_results, file = paste0(path, "/", site, "istat_results.csv"))
```
