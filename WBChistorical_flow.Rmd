...
```
```{r}
function(peakflow_season) {
  monsoon <- peakflow_season %>% 
    filter(season == "Monsoon")
  other <- peakflow_season %>% 
    filter(season == "Other")
  
  
  
  monsoon_day <- mblm::mblm(wateryear_day ~ waterYear, monsoon, repeated = T)$coefficients
  other_day <- mblm::mblm(wateryear_day ~ waterYear, other, repeated = F)$coefficients
  monsoon_magnitude <- mblm::mblm(cfs ~ waterYear, monsoon, repeated = F)$coefficients
  other_magnitude <- mblm::mblm(cfs ~ waterYear, other, repeated = F)$coefficients
