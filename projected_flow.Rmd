...
packages <- c("dataRetrieval", "EGRET", "quantreg", "tidyverse", 
              "rkt", "zyp", "lubridate", "kableExtra", "scales",
              "broom")
...
plotFlowTrend <- function (eList, istat, startDate = NA, endDate = NA, 
  ...
  lines(localSeries$years, localSeries$qSmooth, lwd = lwd, 
        col = col)
  slope_pval <- list(discharge_stat = istat, slope = slope, 
                     slopepct_mag = slopePct,
                     slopepct_mag_interp = "Percent change per year", 
                     p_value = pValue)
  return(slope_pval)
}
...
## 1.3 Set up Filepaths

Now let's get ready to make an EGRET object. 

```{r SiteID, include=FALSE}
filePath <- file.path("EGRETdaily_future_runoff", ID)
fileName <- "daily_input.csv"
```

## 1.4 Set up EGRET object

The next code chunk prepares the gage ID and site name based on the abbreviation you manually entered in Section 1.1. Then, the dataRetrieval package reads in the info available for this gage from NWIS (`INFO`). Currently, the argument `interactive = FALSE` is used to avoid being prompted by the program to enter the info interactively.
...
INFO<- readNWISInfo("09505200","00060", interactive = FALSE)
INFO$shortName <- site
...
The percentiles of daily discharge are provided in CFS first to get an idea of the general distribution of the data.

```{r Historical-Percentiles, echo=FALSE}
knitr::kable(flow_percentiles, format ="html", booktabs=TRUE, longtable = TRUE, caption = paste0("Flow Percentiles for ", siteName), col.names = c("Discharge (cfs)")) %>% 
  kable_styling(latex_options=c("striped","hold_position", "repeat_header"), full_width = F)
```

## 2.2 Hydrograph

The below hydrograph displays the the mean and median from the entire record.
```{r NPS-theme, echo=FALSE, message=FALSE, warning=FALSE}
...
Annual hydrographs (annual discharge sum) for both the calendar year and water year.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
annual_calendar <- Daily %>% 
  select(Year, cfs, Date) %>% 
  mutate(mm = cfs*0.00851016) %>% 
  group_by(Year) %>% 
  dplyr::summarize(mean_cfs = mean(cfs),
            sum_mm = sum(mm))
...
annual_wateryear %>% 
  ggplot(aes(x=waterYear, y = sum_mm)) +
  geom_line() +
  geom_smooth(color = "green", se = F) +
  geom_point() +
  labs(x="Water Year", 
...
for (stat in 1:8) {
  istat <- plotFlowTrend(eList, istat = stat, qUnit = 1, paStart = 10, paLong = 12)
  istat <- c(model = ID, istat)
  istat_results[[length(istat_results)+1]] <- istat
}

istat_results <- reduce(istat_results, bind_rows)
projection_results <- rbind(projection_results, istat_results)
```
...
theil_sen <- senth(peakflow$waterYear, peakflow$wateryear_day, 
                   site = site, 
                   Xlab = "Water Year", 
...
peakflow_season %>% 
  ggplot(aes(x=waterYear, y=wateryear_day, color=month)) +
  geom_point() +
  labs(x="Water Year", y="Days since October 1st") +
  nps_theme2()
...
peakflow_season %>% 
  ...
  geom_smooth(method = sen, se = F) +
  labs(x="Water Year", y="Peak Flow (cfs)") +
...
```{r PeakFlow-lm, echo=FALSE}
lm_peak <- lm(peakflow$wateryear_day~peakflow$waterYear)
summary(lm_peak)
```

``` {r Bind-Regression, echo=FALSE}
theil_bind <- data.frame(model = site, discharge_stat = "peakflow_TS", 
                         slope = theil_sen$slope, 
                         slopepct_mag = (theil_sen$slope)*100, 
                         slopepct_mag_interp = "Estimated Change per Century (number of days)",
                         p_value = theil_sen$p.value)  
lm_results <- tidy(lm_peak)
lm_results <- lm_results[2,] %>% 
  select(term, estimate, p.value) %>% 
  mutate(term = "peakflow_OLS",
         model = site, 
         slopepct_mag = estimate*100, 
         slopepct_mag_interp = "Estimated Change per Century (number of days)") %>% 
  relocate(model, discharge_stat = term, slope = estimate, slopepct_mag, 
           slopepct_mag_interp, p_value = p.value)


get_model_info <- function(data, response_var, predictor_var) {  
    model <- mblm::mblm(as.formula(paste(response_var, "~", predictor_var)), data, repeated = FALSE)  
    corout <- cor.test(data[[predictor_var]], data[[response_var]],                     
                       alternative = "two.sided", method = "kendall", continuity = TRUE)  
    return(list(model = site,
                slope = coefficients(model)[2],
                slopepct_mag = (coefficients(model)[2])*100,
                slopepct_mag_interp = "Estimated Change per Century (number of days or cfs)",
                p_value = corout$p.value))
} 

seasons <- c("Monsoon", "Other")
monsoon_regressions <- list()
for (i in seq_along(seasons)) {  
  season_data <- peakflow_season %>% filter(season == seasons[i])  
  day_info <- get_model_info(season_data, "wateryear_day", "waterYear") 
  day_info$discharge_stat <- paste(seasons[i], ",", "wateryear_day", "~", "waterYear")
  mag_info <- get_model_info(season_data, "cfs", "waterYear")
  mag_info$discharge_stat <- paste(seasons[i], ",", "cfs", "~", "waterYear")
  monsoon_regressions[[i * 2 - 1]] <- day_info   
  monsoon_regressions[[i * 2]] <- mag_info
}

monsoon_regressions <- reduce(monsoon_regressions, bind_rows) %>% 
  relocate(model, discharge_stat, slope, slopepct_mag, 
           slopepct_mag_interp, p_value)
                    
projection_results <- rbind(projection_results, theil_bind, 
                            lm_results, monsoon_regressions)
istat_results <- rbind(istat_results, theil_bind, 
                            lm_results, monsoon_regressions)
```
...
Daily %>% ggplot(aes(x=cfs, y = ..count../sum(..count..)))+
  ...
  theme_bw() + 
  labs(x="Discharge (cfs)", y= "Density (percent)") +
  nps_theme2()
Daily %>% ggplot(aes(cfs))+
  ...
  geom_vline(xintercept = median(Daily$cfs, na.rm = T), color = "red")+
  theme_bw() + 
  labs(x="Discharge (cfs)", y= "Probability") +
  nps_theme2()
```

## 7.3 Saving Data from this Rmd as CSVs
```{r save-csvs, include=FALSE}
futures_folder <- file.path("EGRETdaily_future_runoff", site)
#Daily
write.csv(Daily, file = paste0(futures_folder, "/", site, "Daily.csv"))
#Info
write.csv(INFO, file = paste0(futures_folder, "/", site, "Info.csv"))
#Peak Flow
write.csv(peakflow_season, file = paste0(futures_folder, "/", site, "PeakFlow.csv"))
# Stat results
write.csv(istat_results, file = paste0(futures_folder, "/", site, "istat_results.csv"))
```
