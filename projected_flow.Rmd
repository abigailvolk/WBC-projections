...
plotFlowTrend <- function (eList, istat, startDate = NA, endDate = NA, 
  ...
  lines(localSeries$years, localSeries$qSmooth, lwd = lwd, 
        col = col)
slope_pval <- list(slope = slope, slope_pct = slopePct, p_value = pValue)
return(slope_pval)
}
...
## 3.3 Standard Deviation Plot of the Daily Mean (Log)
...
## 3.5 Flow Statistics with Mann-Kendall Test
...
plotQTimeDaily(eList, lwd = 1, qLower = round(flow_percentiles[8], digits = 3), yearEnd = max(Daily$DecYear))
```
...
senth <- function(x, y, conf = 95, site, Xlab = NULL, Ylab = NULL, legend_loc = "bottomright") {
  ...
  tau <- S / (n * (n - 1) / 2)
  {
    ...
    legend(x = legend_loc, legend=c("Theil-Sen", "Ordinary Least Squares"), lty=c(1,2), cex=0.8)
    return(corout1)
  }
}
```

```{r PeakFlow, echo=FALSE}
peakFlow <- Daily %>%
  group_by(Year) %>%
  filter(cfs == max(cfs))
peakFlow <- peakFlow %>% dplyr::filter(!is.na(Date))
...
write.csv(Daily, file = file.path(filePath, "daily_output.csv"))
#Info
#write.csv(INFO, file = paste0(path, "/", site, "Info.csv"))
#Peak Flow
write.csv(peakFlow, file = file.path(filePath, "DatePeakFlow.csv"))
```
