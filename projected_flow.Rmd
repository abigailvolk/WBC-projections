...
historical_stats %>% ggplot(aes(x=Day)) +
  ...
  theme_bw() + 
  labs(x="Days since January 1st", y="Discharge (cfs)", title=paste0("Summary Hydrograph \n for ", siteName)) +
...
reg_results <- tidy(lm_peak)
reg_results <- reg_results[2,] %>% select(term, estimate, p.value) %>% 
...
reg_results <- rbind(theil_bind, reg_results)
projection_results <- rbind(projection_results, reg_results)
...
projection_results <- rbind(projection_results, models)
```
