...
gw<- 0.09  #set to zero if no ground water contribution expected
...
bias <- 0.3254 #set to 1 if no bias adjustment needed
...
parms <- as.numeric(c(0.99996, 0.3, 0.6))#obtain these parameter values from 
...
unique(month_ex$gcm)
month_ex_hist<-subset(month_ex, gcm == "Historical");month_ex_hist
